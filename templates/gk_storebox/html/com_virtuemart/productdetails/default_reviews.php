	...
	if ($review_editable) {
		...
		<form method="post"
			...
			<?php if($this->allowRating and $review_editable) { ?>
				...
				<div class="rating">
					<label for="vote">
						<span title="Rating" class="vmicon ratingbox" style="display:inline-block;width:55px;"></span>
					</label>
			...
			foreach ($this->rating_reviews as $review) {
				...
				if (!empty($this->rating_reviews) && $review->published) {
					...
					<div class="<?php echo $color ?>">
						...
						<span class="vote"><span class="vmicon" style="background-position: 0 <?php echo ((5 - (int)$review->review_rating) * -10) . 'px'; ?>;" title="Rating: <?php echo (int)$review->review_rating; ?>/5"></span></span>
