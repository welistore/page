...
class InstallationModelDatabase extends JModelBase
{
	...
	public function populateDatabase($db, $schema)
	{
		...
		$vm_additiona_ops = false;
		
		if(stripos($buffer, 'virtuemart_categories_') !== FALSE) {
			$vm_additiona_ops = true;
		}
		...
		if($vm_additiona_ops) {
			$db->setQuery('SELECT config, virtuemart_config_id FROM #__virtuemart_configs');
			
			if($rows = $db->loadObjectList()) {
				foreach($rows as $item) {
					$config = $item->config;
					$config = str_replace('active_languages=[\"en-GB\"]', 'active_languages=[\"'.$options->getTag().'\"]', $config);
					$config = str_replace('active_languages=["en-GB"]', 'active_languages=["'.$options->getTag().'"]', $config);
					$config = str_replace('active_languages=a:1:{i:0;s:5:"en-GB";}', 'active_languages=a:1:{i:0;s:5:"'.$options->getTag().'";}', $config);
					
					$db->setQuery('UPDATE #__virtuemart_configs SET config = \''.$config.'\' WHERE virtuemart_config_id = '.$item->virtuemart_config_id.';');
